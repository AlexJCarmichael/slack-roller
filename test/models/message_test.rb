...
class MessageTest < ActiveSupport::TestCase
  ...
  test "message parsing with no modifiers" do
    assert_equal({ times_rolled: "3", sides_to_die: "d6", dropped_die: nil, attachment: nil },
                 @msg1.parse_message)
  end

  test "message parsing with add and subtract modifiers" do
    assert_equal({ times_rolled: "2", sides_to_die: "d6", dropped_die: nil, attachment: "+ 2" },
                 @msg2.parse_message)
    assert_equal({ times_rolled: "4", sides_to_die: "d8", dropped_die: nil, attachment: "- 3" },
                 @msg3.parse_message)
  end

  test "message parsing with add and subtract modifiers with drop high and low" do
    assert_equal({ times_rolled: "3", sides_to_die: "d6", dropped_die: "drop", attachment: "+ 2" },
                 @msg4.parse_message)
    assert_equal({ times_rolled: "4", sides_to_die: "d8", dropped_die: "drop", attachment: "- 3" },
                 @msg5.parse_message)
  end
